...
function print_menu(){
	...
	if [ -d "${UNM_SERV_BIN_DIR}" ]; then
		...
		if [ -z "${UNM_SERV_PID}" ] || [ "${UNM_SERV_PID}" == "0" ]; then
			...
			UNM_SERV_IP="$(curl -fsL "https://ifconfig.me" || curl -fsL "https://ipinfo.io/ip" || echo "127.0.0.1")"
...
function install_unm_server() {
	...
	local UNM_SERV_IP="$(curl -fsL "https://ifconfig.me" || curl -fsL "https://ipinfo.io/ip" || echo "127.0.0.1")"
